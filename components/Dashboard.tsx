  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 animate-fadeIn">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        <div>
          <h1 className="text-3xl font-bold text-pomegranate-900">{t('dashboard.title')}</h1>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-12 text-center">
          ...
          <h3 className="text-lg font-medium text-gray-900">{t('dashboard.noPolicies')}</h3>
          <p className="text-gray-500 mt-2 mb-6">{t('dashboard.noPoliciesDesc')}</p>
          <button onClick={onShopNew} className="text-pomegranate-600 font-semibold hover:underline">{t('dashboard.getQuote')}</button>
        </div>
      ) : (
              ...
              <div className="flex flex-col md:flex-row">
                ...
                <div className="flex-1 p-6">
                  <div className="flex flex-col md:flex-row justify-between md:items-start gap-4 mb-6">
                    <div>
                      ...
                      <h3 className="text-xl font-bold text-pomegranate-900">
                        {policy.request.userType === 'OWNER' ? t('dashboard.homeowner') : t('dashboard.tenant')} {t('dashboard.policy')}
                      </h3>
                      <p className="text-sm text-gray-500">{policy.quoteResult?.schemeName || 'Standard Plan'}</p>
                    </div>
                    <div className="text-left md:text-right">
                      <p className="text-sm text-gray-500">{t('dashboard.premiumPaid')}</p>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-6 border-t border-gray-100 pt-6">
                    <div>
                      <p className="text-xs text-gray-400 uppercase font-semibold mb-1">{t('dashboard.effectiveDate')}</p>
                      <p className="text-sm font-medium text-gray-700">{new Date(policy.startDate).toLocaleDateString()}</p>
                    </div>
                    <div>
                      <p className="text-xs text-gray-400 uppercase font-semibold mb-1">{t('dashboard.expirationDate')}</p>
                      <p className="text-sm font-medium text-gray-700">{new Date(policy.endDate).toLocaleDateString()}</p>
                    </div>
                    <div className="md:col-span-2">
                      <p className="text-xs text-gray-400 uppercase font-semibold mb-1">{t('dashboard.coverageHighlights')}</p>
